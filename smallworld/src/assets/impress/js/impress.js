  ...
  var impressConsole = (window.impressConsole = function (rootId) {
    ...
    var open = function () {
      ...
      if (consoleWindow && !consoleWindow.closed) {
        ...
        registerKeyEvent([37], window.impress().prev);

        // 34: pg down, 39: right, 40: down
        registerKeyEvent([39], window.impress().next);
    ...
    var resize = function () {
      ...
      if (preViewWidth <= Math.floor(slideViewWidth * preViewMinimumFactor)) {
        ...
        slideViewHeight = Math.floor(
          (views.clientHeight - delta - preViewGap) / (1 + preViewMinimumFactor)
        );
...
(function (document, window) {
  ...
  document.addEventListener("touchend", function () {
    ...
    if (
      Math.abs(totalDiff) > window.innerWidth / 5 &&
      totalDiff * lastDX <= 0
    ) {
